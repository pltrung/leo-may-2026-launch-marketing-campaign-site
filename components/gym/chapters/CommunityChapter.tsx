...
export default function CommunityChapter() {
  const locale = useLocale();
  const { openPricingModal } = useGymNav();
  const m = getMessages(locale as "en" | "vi").gym.chapter3;
  const pricingTitle = getMessages(locale as "en" | "vi").gym.pricingModal.title;
