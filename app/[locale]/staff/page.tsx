...
export default function StaffPage() {
  ...
  if (!staff) {
    return <RouteSetterLoginForm locale={locale} />;
  }
      ...
      <main className="p-4 pb-8 space-y-6 max-w-2xl mx-auto">
        ...
        <section className="rounded-xl bg-slate-800 border border-slate-700 p-4">
          ...
          {!hasAnsweredAttendance ? (
            <>
              <p className="text-slate-200 mb-4">{m.areYouWorkingToday}</p>
            ...
            <p className="text-slate-200">
              {m.youAreMarked} <strong className={isIn ? "text-emerald-400" : "text-slate-400"}>{attendance?.status}</strong> {m.forToday}
            </p>
          )}
        </section>
