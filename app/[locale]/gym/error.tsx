...
export default function GymError({
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
      console.error("[GymError] Route error:", error);
    }
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-6" style={{ background: "#0B0B0F", color: "#e5e5e5" }}>
      <div className="max-w-md w-full rounded-2xl border border-white/10 bg-white/5 p-6 text-center backdrop-blur-sm">
        <h2 className="text-lg font-semibold mb-2">System temporarily unavailable</h2>
        <p className="text-sm opacity-80 mb-4">
          The gym page encountered an error. Please try again or return to the homepage.
        </p>
