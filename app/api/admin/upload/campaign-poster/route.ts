...
/**
 * POST /api/admin/upload/campaign-poster
 * Same storage as product photos; path prefixed for clarity. Analytics access only.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const path = `campaign-posters/${crypto.randomUUID()}.${ext}`;
    ...
    if (uploadErr) {
      console.error("campaign poster upload error", uploadErr);
      return NextResponse.json(
