...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: existing, error: rowError } = await supabase
      ...
      .select("id, auth_id, email, phone, full_name, tier, waiver_signed, waiver_signed_at, created_at, member_code, membership_status, membership_expires_at, visits_remaining, profile_photo_url, id_number, date_of_birth, instagram_handle, gender, address, id_verified_from_cccd, current_streak, best_streak")
      .eq("auth_id", user.id)
      .maybeSingle();
