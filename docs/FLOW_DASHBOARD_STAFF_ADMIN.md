...
- **Auth**: `AdminAuthProvider` in admin layout. Session must be one of `ADMIN_EMAILS`. All admin API routes that need auth call `getAdminFromRequest(req)` and return 401 if not admin.
